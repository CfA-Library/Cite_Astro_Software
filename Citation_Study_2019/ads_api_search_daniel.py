...
keywords_list = (open('updated_keywords_list.txt','r')).read()
...
def keywordquery(body,daterange,devkey):
    ...
    for i in range (0,loop+1):
        ...
        query = 'body:%22'+ urllib.quote_plus(body) + '%22%20pubdate:%5B' + daterange + '%5D' + '&hl=true&indent=true&hl.fl=title,abstract,full,body,ack,doi,bibcode&fl=id,bibcode,alternate_bibcode,bibgroup,bibstem,identifier,doi,year,ack,first_author,title&rows='+str(rows)+'&start='+str(startnum)
        ...
        for x in docs:
            ...
            row = [body]+[highlightclean]+[bibcode]+[alternate_bibcode_clean]+[bibgroupclean]+[bibstem]+[identifier_clean]+[doiclean]+[year]+[first_author]+[titleclean]
            wr.writerow(row)
...
wr.writerow(['Keyword']+['Highlight']+['Bibcode']+['Alternate Bibcode']+['BibGroup']+['BibStem']+['Identifier']+['DOI']+['Pub. Year']+['Author']+['Title'])
