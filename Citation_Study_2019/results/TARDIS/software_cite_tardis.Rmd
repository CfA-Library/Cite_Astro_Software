...
TARDIS <- filter(TARDIS, Publication_year %in% valid_years)

# remove confounds
confounds <- c("@tardis.byu.edu","tardis.ln.byu.edu","@tardis.pha.jhu.edu")

TARDIS <- filter(TARDIS, !XML_tag1_content %in% confounds)
```
XML tag analysis
```{r}
# how many unique XML tags were used?
length(unique(TARDIS$XML_tag2)) # 3
...
total_TARDIS_files <- length(unique(TARDIS$File_name))  # 3
total_TARDIS_files
...
cite_attempt_TARDIS <- length(unique(TARDIS_attempt_ref$File_name)) # 0
cite_attempt_TARDIS

# proportion of TARDIS mentions with an attempt at citation
cite_attempt_TARDIS / total_TARDIS_files # 0
