...
```{r}
valid_years <- c(2007:2018)
RADMC3D <- filter(RADMC3D, Publication_year %in% valid_years)
```

XML tag analysis
```{r}
# how many unique XML tags were used?
length(unique(RADMC3D$XML_tag2)) # 13

# what were the tags?
unique(RADMC3D$XML_tag2)
...
ID_only <- filter(RADMC3D, ID == 1)
ID_count <- length(unique(ID_only$File_name))
ID_count # 112

# proportion of RADMC3D formal citations containing an identifier
ID_count / total_RADMC3D_files # 0.56
