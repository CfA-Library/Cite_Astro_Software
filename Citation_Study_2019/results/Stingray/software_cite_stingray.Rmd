...
```{r}
valid_years <- c(2016:2018)
Stingray <- filter(Stingray, Publication_year %in% valid_years)

# remove confounds
confounds <- c("Stingray Nebula","Stingray nebula","stingray nebula","The Stingray","the Stingray","the stingray","Stingray Light Curve","Stingray light curve","stingray light curve")

Stingray <- filter(Stingray, !XML_tag1_content %in% confounds)
```

XML tag analysis
```{r}
# how many unique XML tags were used?
length(unique(Stingray$XML_tag2)) # 8
...
total_Stingray_files <- length(unique(Stingray$File_name))  # 3
...
cite_attempt_Stingray / total_Stingray_files # 0.3333333
...
formal_cite_Stingray / total_Stingray_files # 0.3333333

# how many formal citations contained an "identifier"
ID_only <- filter(Stingray, ID == 1)
ID_count <- length(unique(ID_only$File_name))
ID_count # 0

# proportion of Stingray formal citations containing an identifier
ID_count / total_Stingray_files # 0
